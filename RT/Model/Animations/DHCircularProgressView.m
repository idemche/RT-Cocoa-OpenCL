...
- (void)drawRect:(NSRect)dirtyRect
{
	...
	NSRect innerCircleGroup = NSMakeRect(NSMinX(frame) + w / 3, NSMinY(frame) + h / 3, NSWidth(frame) - (w - 100), NSHeight(frame) - (h - 100));
	...
	//// Inner Circle Group
	{
		...
		[progressImage drawInRect: NSMakeRect(floor(NSMinX(imageCircleRect) + 2 + 0.5), ceil(NSMinY(imageCircleRect) + 2 - 0.5), progressImage.size.width, progressImage.size.height) fromRect: NSZeroRect operation: NSCompositingOperationSourceOver fraction: 1];
