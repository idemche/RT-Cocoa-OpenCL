...
/*   Updated: 2017/10/17 20:14:01 by hshakula         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "rt.h"

static t_material	get_material_type(char *material)
{
	...
	{
		ft_warning("invalid material type, default DIFF");
		return (DIFF);
	}
}
														...
														t_json_material *m)
{
		...
		a->objects[i].material == GLOSSY_REFR)
	{
		...
		{
			ft_warning("invalid glossiness, default 0");
		...
		a->objects[i].material == GLOSSY_REFR)
	{
		...
		{
			ft_warning("invalid n (refractive index), default 2.5");
...
static void			parse_material_2(t_info *a, int i, t_json_scene *js,
															t_json_material *m)
{
	a->objects[i].material = get_material_type(m->material->valuestring);
	...
	if (a->objects[i].material != DIFF)
	{
		...
		if (!cJSON_IsNumber(m->kd) || m->kd->valuedouble < 0
							|| m->kd->valuedouble > 1)
		{
			ft_warning("invalid kd coefficient, default 0");
			a->objects[i].kd = 0;
		}
		else
															...
															t_json_scene *js)
{
	...
	{
		ft_warning("invalid absorption field, default (0, 0, 0)");
...
void				parse_material(t_info *a, int i, t_json_scene *js)
{
	...
	if (m.u)
		parse_point(&a->objects[i].upsilon, m.u);
	else
	...
	if (a->objects[i].type == SPHERE)
		parse_emission(&a->objects[i].emission, m.emission);
	if (m.material)
